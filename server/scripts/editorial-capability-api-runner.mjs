...
export async function runEditorialCapabilityApiRunner(options = {}) {
...
async function runCli() {
  ...
  const result = await runEditorialCapabilityApiRunner(args);
...
if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  runCli().catch((error) => {
    const payload = {
      ok: false,
      error: {
        code: error?.code ?? "EDITORIAL_CAPABILITY_API_RUNNER_FAILED",
        message: error?.message ?? "Editorial capability API runner failed"
      },
      timestamp: new Date().toISOString()
    };
