...
const REGISTRAR_EXPORT_NAME = "registerServices";

function shouldAttemptServiceRegistrar(manifest) {
  const capabilities = Array.isArray(manifest?.capabilities) ? manifest.capabilities : [];
  const runtimeEntrypoint = manifest?.runtime?.services;

  return capabilities.includes("service") || typeof runtimeEntrypoint === "string";
}
...
function resolveRegistrarEntrypoint(manifest) {
  ...
  if (typeof runtimeEntrypoint !== "string" || runtimeEntrypoint.length === 0) {
    return {
      ...
      message: `Module '${manifest?.id}' declares services but runtime.services is not configured`
    };
  }
  ...
  if (typeof moduleDir !== "string" || moduleDir.length === 0) {
    return {
      ok: false,
      code: "SERVICE_REGISTRAR_SOURCE_MISSING",
  ...
  if (relativePath.startsWith("..") || path.isAbsolute(relativePath)) {
    return {
      ok: false,
      code: "SERVICE_REGISTRAR_PATH_INVALID",
      message: `Module '${manifest?.id}' runtime.services must resolve within the module directory`,
...
export async function registerServicesForDiscoveredModules({
  moduleRegistry,
  serviceRegistry,
  registrationContext
}) {
  ...
  for (const { manifest } of moduleRegistry.list()) {
    if (!shouldAttemptServiceRegistrar(manifest)) {
      continue;
    }
    ...
    try {
      ...
      diagnostics.push({
        code: "SERVICE_REGISTRAR_IMPORT_FAILED",
        message: `Failed loading service registrar for module '${manifest.id}'`,
        moduleId: manifest.id,
    ...
    if (typeof registrar !== "function") {
      diagnostics.push({
        code: "SERVICE_REGISTRAR_INVALID_EXPORT",
        message: `Service registrar for module '${manifest.id}' must export '${REGISTRAR_EXPORT_NAME}'`,
        moduleId: manifest.id,
        entrypoint: resolvedEntrypoint.entrypoint,
        resolvedPath: resolvedEntrypoint.resolvedPath
      });
    ...
    try {
      ...
      diagnostics.push({
        code: "SERVICE_REGISTRAR_FAILED",
        message: `Service registrar failed for module '${manifest.id}'`,
        moduleId: manifest.id,
        entrypoint: resolvedEntrypoint.entrypoint,
        resolvedPath: resolvedEntrypoint.resolvedPath,
